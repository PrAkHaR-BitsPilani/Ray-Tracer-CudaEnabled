...
namespace glm {
    ...
    public:
        ...
        __host__ __device__ vec3(float a) { e[0] = e[1] = e[2] = a; }
    ...
    __host__ __device__ inline float radians(float angle) {
        return angle * pi / 180.0f;
    }

    __host__ __device__ bool nearZero(glm::vec3 e)
    {
        const auto s = 1e-8;
        return (fabs(e.x()) < s) && (fabs(e.y()) < s) && (fabs(e.z()) < s);
    }

    __host__ __device__ inline float length(const vec3& e){
        return sqrt(e[0] * e[0] + e[1] * e[1] + e[2] * e[2]);
    }

    __host__ __device__ inline float length_squared(const vec3& e) {
        return e[0] * e[0] + e[1] * e[1] + e[2] * e[2];
    }
